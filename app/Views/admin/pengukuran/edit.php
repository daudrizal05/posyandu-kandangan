    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card card-warning">
                ...
                <form action="<?= site_url('admin/pengukuran/update/'.$pengukuran['id']) ?>" method="post">
                    <?= csrf_field() ?>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="posyandu_id" name="posyandu_id" required>
                                        ...
                                        <?php foreach($posyandu as $p): ?>
                                            <option value="<?= $p['id'] ?>" <?= old('posyandu_id', $pengukuran['posyandu_id']) == $p['id'] ? 'selected' : '' ?>><?= esc($p['nama_posyandu']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="balita_id" name="balita_id" required>
                                        ...
                                        <?php foreach($balita as $b): ?>
                                            <option value="<?= $b['id'] ?>" <?= old('balita_id', $pengukuran['balita_id']) == $b['id'] ? 'selected' : '' ?>><?= esc($b['nik']) ?> - <?= esc($b['nama_balita']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" id="tanggal_pengukuran" name="tanggal_pengukuran" value="<?= old('tanggal_pengukuran', $pengukuran['tanggal_pengukuran']) ?>" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="berat_badan">Berat Badan (kg) <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" class="form-control" id="berat_badan" name="berat_badan" value="<?= old('berat_badan', $pengukuran['berat_badan']) ?>" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="tinggi_badan">Tinggi Badan (cm) <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" class="form-control" id="tinggi_badan" name="tinggi_badan" value="<?= old('tinggi_badan', $pengukuran['tinggi_badan']) ?>" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="keterangan" name="keterangan" rows="2" placeholder="Opsional..."><?= old('keterangan', $pengukuran['keterangan']) ?></textarea>
                            <small class="text-muted d-block mt-1"><i class="fas fa-info-circle"></i> Catatan: Status Gizi dan Nilai Z-Score (BB/U, TB/U, BB/TB) akan dihitung ulang secara otomatis berdasarkan standar WHO 2006 setelah data diupdate.</small>
                        </div>

                        <div class="form-group">
                            <label for="keterangan">Keterangan / Intervensi</label>
                            <textarea class="form-control" id="keterangan" name="keterangan" rows="2"><?= old('keterangan', $pengukuran['keterangan']) ?></textarea>
                        </div>
                    </div>
        ...
        <div class="col-md-12 mt-4">
            <div class="card card-outline card-info">
                <div class="card-header">
                    <h3 class="card-title">Riwayat Pengukuran Balita Ini (Sub Tabel)</h3>
                </div>
                        ...
                        <table class="table table-bordered table-striped text-sm mb-0">
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    ...
                                    <tr <?= $r['id'] == $pengukuran['id'] ? 'class="bg-warning"' : '' ?>>
                                        <td><?= $no++ ?></td>
