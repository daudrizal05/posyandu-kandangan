...
<head>
  ...
  <style>
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
    ...
    /* page */
    .page-header{background:linear-gradient(135deg,#1344b0 0%,#0e337a 100%);padding:40px 20px;color:#fff;}
    ...
    .filter-bar input{flex:1;padding:10px 14px;border:1.5px solid var(--border);border-radius:8px;font-size:13.5px;font-family:'Poppins',sans-serif;color:var(--text);}
    .filter-bar input:focus{outline:none;border-color:var(--blue);}
    .filter-bar button{padding:10px 22px;background:var(--blue);color:#fff;border:none;border-radius:8px;font-size:13.5px;font-family:'Poppins',sans-serif;font-weight:600;cursor:pointer;}
    .filter-bar button:hover{background:var(--blue-dk);}
    /* Table */
    ...
    .table-head{background:var(--blue);color:#fff;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;}
    .table-head h3{font-size:15px;font-weight:600;}
    .table-head .count{font-size:12px;opacity:.75;}
    ...
    .pagination a,.pagination span{display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:7px;font-size:13px;margin:0 2px;border:1px solid var(--border);color:var(--text);transition:all .18s;}
...
<div class="page-header">
  <div class="wrap">
    ...
    <h1><i class="fas fa-child" style="margin-right:10px;"></i>Data Balita – <?= esc($posyandu['nama_posyandu']) ?></h1>
    <p>Daftar seluruh balita yang terdaftar di posyandu ini.</p>
  </div>
</div>
...
<div class="page-content">
  ...
  <form class="filter-bar" method="get">
    <input type="text" name="keyword" placeholder="Cari NIK atau nama balita..." value="<?= esc($keyword ?? '') ?>">
    <button type="submit"><i class="fas fa-search"></i> Cari</button>
    <?php if (!empty($keyword)): ?>
      <a href="<?= site_url('posyandu-' . $posyandu['id'] . '/balita') ?>" style="padding:10px 18px;color:var(--muted);font-size:13.5px;">Reset</a>
    <?php endif; ?>
  </form>

  <div class="table-wrap">
    <div class="table-head">
      <h3><i class="fas fa-list" style="margin-right:8px;"></i>Daftar Balita</h3>
    </div>
    <?php if (!empty($balita)): ?>
    <table>
      <thead>
      ...
      <tbody>
        ...
        <tr>
          <td><?= $no++ ?></td>
          <td><?= esc($b['nik']) ?></td>
          <td><strong><?= esc($b['nama_balita']) ?></strong></td>
          <td>
            <?php if ($b['jenis_kelamin'] === 'L'): ?>
              <span class="badge badge-m"><i class="fas fa-mars"></i> L</span>
            <?php else: ?>
              <span class="badge badge-f"><i class="fas fa-venus"></i> P</span>
            <?php endif; ?>
          </td>
          ...
          <td>
            ...
            <?php else: ?>
              <span class="badge badge-lain"><?= ucfirst($b['status']) ?></span>
            <?php endif; ?>
          </td>
        </tr>
        <?php endforeach; ?>
    ...
    <div class="empty">
      ...
      <p>Belum ada data balita<?= !empty($keyword) ? ' dengan pencarian "<strong>' . esc($keyword) . '</strong>"' : '' ?>.</p>
    </div>
    <?php endif; ?>
  </div>
</div>
